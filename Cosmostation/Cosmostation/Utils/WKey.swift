...
class WKey {
    ...
    static func getMasterKeyFromWords(_ m: [String]) -> HDPrivateKey {
        let data = try! Mnemonic.seed(mnemonic: m)
        return HDPrivateKey(seed: data, network: .testnetBCH)
    }
    ...
    static func getHDKeyDpAddressWithPath(_ masterKey:HDPrivateKey, path:Int, chain:ChainType, _ newbip:Bool) -> String {
        do {
            ...
            return getPubToDpAddress(childKey!.privateKey().publicKey().data.dataToHexString(), chain)
        } catch {
            return ""
        }
    }
