    ...
    func passwordResponse(result: Int) {
        if (result == PASSWORD_RESUKT_OK) {
            if (pageHolderVC.chainType! == ChainType.COSMOS_MAIN || pageHolderVC.chainType! == ChainType.IRIS_MAIN || pageHolderVC.chainType! == ChainType.KAVA_MAIN ||
                    ...
                    pageHolderVC.chainType! == ChainType.CERTIK_MAIN || pageHolderVC.chainType! == ChainType.IOV_TEST) {
        ...
        DispatchQueue.global().async {
            ...
            do {
                ...
                if (self.pageHolderVC.chainType! == ChainType.COSMOS_MAIN || self.pageHolderVC.chainType! == ChainType.KAVA_MAIN || self.pageHolderVC.chainType! == ChainType.KAVA_TEST ||
                        ...
                        self.pageHolderVC.chainType! == ChainType.CERTIK_TEST) {
    ...
    func onGenBnbSendTx() {
        ...
        DispatchQueue.global().async {
            ...
            if (self.pageHolderVC.chainType! == ChainType.BINANCE_MAIN) {
                ...
                wallet = Wallet(privateKey: pKey!.privateKey().data.hexEncodedString(), endpoint: BinanceChain.Endpoint.mainnet)
                
            } else {
                //For Binance testent send
                binance = BinanceChain(endpoint: BinanceChain.Endpoint.testnet)
                pKey = WKey.getHDKeyFromWords(words, self.pageHolderVC.mAccount!)
                wallet = Wallet(privateKey: pKey!.privateKey().data.hexEncodedString(), endpoint: BinanceChain.Endpoint.testnet)
            }
