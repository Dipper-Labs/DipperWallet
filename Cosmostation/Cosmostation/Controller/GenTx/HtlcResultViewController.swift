...
//        print("onCreateHtlcSwap")
        DispatchQueue.global().async {
            ...
            if (self.chainType == ChainType.BINANCE_MAIN) {
                ...
                let wallet = Wallet(privateKey: pKey.privateKey().data.hexEncodedString(), endpoint: BinanceChain.Endpoint.mainnet)
                ...
                let pKey = WKey.getHDKeyFromWords(words, self.account!)
                let wallet = Wallet(privateKey: pKey.privateKey().data.hexEncodedString(), endpoint: BinanceChain.Endpoint.testnet)
